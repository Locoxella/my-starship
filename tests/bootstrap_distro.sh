...
if command -v dnf &>/dev/null; then
    ...
    dnf install -y curl git tar xz which zsh fish util-linux procps-ng fontconfig fastfetch
elif command -v apt-get &>/dev/null; then
    echo "[+] Detected APT (Ubuntu/Debian family)..."
    export DEBIAN_FRONTEND=noninteractive
    apt-get update -y
    apt-get install -y curl git tar xz-utils zsh fish procps fontconfig ca-certificates fastfetch || \
    ...
    pacman -Syu --noconfirm curl git tar xz zsh fish which procps-ng fontconfig ca-certificates fastfetch
elif command -v zypper &>/dev/null; then
    echo "[+] Detected Zypper (openSUSE family)..."
    zypper refresh
    zypper install -y curl git tar xz which zsh fish procps fontconfig ca-certificates fastfetch
elif command -v apk &>/dev/null; then
    echo "[+] Detected APK (Alpine Linux family)..."
    apk add --no-cache bash curl git tar xz zsh fish procps fontconfig shadow fastfetch
else
